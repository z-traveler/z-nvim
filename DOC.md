...
默认优先使用 `lemonade` 作为 SSH 场景的双向剪贴板 provider；只有检测到远端 `127.0.0.1:2489` reverse tunnel 已连通，或显式设置 `Z_NVIM_CLIPBOARD=lemonade` 时才启用。不会自动切到 `xclip + VcXsrv`，避免每次 paste 都出现明显延迟。

推荐链路：

```text
Windows clipboard <-> Windows lemonade server <-> SSH reverse tunnel <-> remote lemonade client <-> Neovim
```

Windows 侧启动一次 server：
...
```powershell
ssh -R 2489:127.0.0.1:2489 <remote>
```

也可以写入 Windows 的 SSH config：
...
远端需要有 `lemonade` client，并使用默认 `host=localhost` / `port=2489`。验证命令：

```bash
printf 'hello-from-remote' | lemonade copy
lemonade paste
```
...
| 环境变量 | 说明 |
|----------|------|
| `Z_NVIM_CLIPBOARD=lemonade` | 强制优先使用 `lemonade`（默认行为，前提是远端有 `lemonade`） |
| `Z_NVIM_CLIPBOARD=xclip` | 显式使用 `xclip + X11` fallback |
| `Z_NVIM_X11_DISPLAY=<host>:0.0` | 显式设置 X11 display，并启用 `xclip` fallback |
| `Z_NVIM_LEMONADE_HOST=127.0.0.1` | `lemonade` reverse tunnel 检测 host |
| `Z_NVIM_LEMONADE_PORT=2489` | `lemonade` reverse tunnel 检测 port |
| `Z_NVIM_LEMONADE_DETECT_TIMEOUT=0.1s` | `lemonade` tunnel 检测超时 |
| `Z_NVIM_CLIPBOARD=off` | 不设置自定义 clipboard provider |
| `Z_NVIM_CLIPBOARD_TIMEOUT=0.2s` | 调整 `xclip` fallback 的 paste 超时，默认 `1s` |

`xclip + VcXsrv` 只作为 fallback；它能双向同步，但延迟明显高于专用 clipboard bridge。
