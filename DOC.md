...
默认使用 `xsel + X11` 作为 SSH 到 Windows/VcXsrv 场景的双向剪贴板 provider。`z-nvim` 不做网络探测；只在当前环境已有 `DISPLAY` 且远端能找到 `xsel` 时设置 `vim.g.clipboard = "xsel"`，避免启动时引入额外延迟。

推荐链路：

```text
Windows clipboard <-> VcXsrv X Server <-> SSH remote DISPLAY <-> xsel <-> Neovim
```

Windows 侧保持 VcXsrv 运行，并允许远端主机连接 X server。远端 shell 由 `z-bootstrap` 的 `.bashrc.user` 派生 `DISPLAY`：
